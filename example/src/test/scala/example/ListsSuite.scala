...
class ListsSuite extends FunSuite {
  ...
  test("max of negatives is zero") {
    assert(max(List(-1, -4, -3, 0, -2, -1)) === 0)
  }
  
  test("max of duplicate elements") {
    assert(max(List(1,3,1,4,3,4,2,1,2)) === 4)
  }
}
