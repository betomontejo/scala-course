...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (for ( (x,y) <- (w groupBy (ch => ch.toLower)).toList ) yield (x, y.length)) sortBy(x => x._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
	...
	def explode(l: List[(Char, Int)]) : List[List[(Char, Int)]] =
	 ( for (lst <- l) yield expand(lst) ).toList
    ...
    combine(explode(occurrences)) map (_.filter( x => x._2 > 0))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    (for(elem <- x ) yield (elem._1, elem._2 - yMap(elem._1) )).filter( n => n._2 > 0)
  }
